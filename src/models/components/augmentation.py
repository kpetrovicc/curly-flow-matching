...
class SquaredL2Reg(RegularizationFunc):
    def forward(self, t, x, dx, context) -> torch.Tensor:
        ...
        return torch.pow(torch.norm(to_return, p=2, dim=1), 2) #.unsqueeze(-1)  # dim mismatch added unsqueeze(-1)
...
class JacobianFrobeniusReg(RegularizationFunc):
    def forward(self, t, x, dx, context) -> torch.Tensor:
        ...
        jac = _get_minibatch_jacobian(dx, x)
        context.jac = jac
        return _batch_root_mean_squared(jac) #.unsqueeze(-1)  # dim mismatch added unsqueeze(-1)
...
class CNFReg(RegularizationFunc):
    ...
    def forward(self, t, x, dx, context):
        ...
        return -self.trace_estimator(dx, x) #.unsqueeze(-1) + 0 * x # dim mismatch added unsqueeze(-1)


class SquaredL2RegWithRespectToVector(RegularizationFunc):
    ...
    def forward(self, t, x, dx, context) -> torch.Tensor:
        to_return = dx.view(dx.shape[0], -1)
        return torch.pow(
            torch.norm(to_return - self.ut(x), p=2, dim=1), 2
        )  # .unsqueeze(-1)  # dim mismatch added unsqueeze(-1)
...
class AugmentationModule(nn.Module):
    ...
    def __init__(
        ...
        squared_l2_vector_reg: float = 0.0,
        ...
        if squared_l2_vector_reg > 0.0:
            names.append("squared_L2_vector")
            coeffs.append(squared_l2_vector_reg)
            regs.append(SquaredL2RegWithRespectToVector(ut))
        if l1_reg > 0.0:
            names.append("L1")
